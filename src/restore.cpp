...
CallStack restore_stack() {
    ...
    for (int i = 0; i < call_stack_size; ++i) {
        ...
        CallStackEntry *csp = &call_stack.entries[i];
        CodePos *cur_pos = &csp->pc;
        CodePos *ret_pos;
        if (i == 0) ret_pos = &csp->pc;
        else ret_pos = &call_stack.entries[i-1].pc;
        Array32 *locals = &csp->locals;
        Array32 *value_stack = &csp->value_stack;
        LabelStack *label_stack = &csp->label_stack;

        fread(&cur_pos->fidx, sizeof(uint32_t), 1, fp);
        fread(&cur_pos->offset, sizeof(uint32_t), 1, fp);
